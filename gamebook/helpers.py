...
def statsdisplay(trait1, trait2, trait3):
    ...
    display = ' ------------------------------\n'
    display += '|'+trait1.get_name()+': ' + str(trait1.get_value()) + '|'+trait2.get_name()+': '+str(trait2.get_value())+\
...
def roll(trait, cond):
    ...
    cond - the condition that determines wheter the player is lucky or not. cond can get only 2 inputs:
